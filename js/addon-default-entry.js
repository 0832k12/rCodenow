...
class DevTools {
  ...
  async addContextMenus() {
    ...
    let s = async (block, t, bg) => {
      ...
      if (bg) {
        while (ohtml.className.animVal !== 'blocklyBlockCanvas') ohtml = ohtml.parentElement;
      }
      bb = ohtml.getBBox();
      html = $(ohtml).clone()[0];
      ...
      for (let i = 0; i < eimg.length; i++) {
        ...
        if (!link.startsWith('data')) {
          if (!linkList[link]) linkList[link] = await getBase64Image(link);
      ...
      if (bg) {
        ...
        for (let i = 0; i < cl.length; i++) {
          let d = $(cl[i]);
          d.attr('transform', "translate(".concat(x[i] - mx, ",").concat(y[i] - my, ")")).attr('style', '');
        }
      }
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      let svg = "<svg width=\"999999\" height=\"999999\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n        <style xmlns=\"http://www.w3.org/2000/svg\">\n          .blocklyText {\n              fill: #fff;\n              font-family: \"Helvetica Neue\", Helvetica, sans-serif;\n              font-size: 12pt;\n              font-weight: 500;\n          }\n          .blocklyEditableText>text {\n              fill: #575E75;\n          }\n          .blocklyDropdownText {\n            fill: #fff !important;\n          }\n        </style>\n        ".concat(html.outerHTML.replaceAll('&nbsp;', ' '), "\n        </svg>");
      const img = new Image();
